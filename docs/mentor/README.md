...
- 帮你把“匿名发起分析 -> 后端执行 -> SSE 推送 -> 报告落库 -> 前端展示”这条主链路讲清楚
- 帮你理解项目当前的关键能力：Source Intelligence V2、匿名分析、本地缓存、SQLite checkpoint、SSE 恢复
...
3. `POST /api/v1/analyze`、`/reports/*`、SSE 流在 `main` 上默认匿名可用，不依赖登录
...
- 准备接手报告页、SSE、pipeline、source 扩展、本地部署等模块
