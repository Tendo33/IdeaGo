...
- Prefer same-origin relative API paths for hosted builds.
- Use `VITE_*` only for browser-safe build-time values.
- Docker build args may pass `VITE_API_BASE_URL`, Supabase public values, and
  Turnstile site key.
- Never expose service-role keys, Stripe secrets, LinuxDo secrets, or raw
  provider API keys through Vite variables.
