...
- 匿名提交想法
- 在拿到真实报告 id 之前，会先经过临时的 `/reports/new` 过渡态
- SSE 实时进度流，且有最大重连次数与 `/status` fallback
- 本地 file cache 保存报告历史
- 报告详情与 Markdown 导出
- 前端会自动生成稳定的匿名 `X-Session-Id`
...
提交想法后，前端会先走 `/reports/new` 这个匿名创建过渡态，随后再切换到真实的
`/reports/:id`。这个过渡态是当前主流程的一部分。

当前已经内建的运行时保护：

- 生产日志默认关闭 `backtrace` 与 `diagnose`
- API 响应默认带基础 `Content-Security-Policy`
- `/reports*` 限流按读、状态、流、变更四类分桶
- 报告页面的决策优先 section 顺序固定为 recommendation、why-now、pain、commercial、
  whitespace、competitors、evidence、confidence
...
匿名 session 标识和 SSE 恢复不需要额外前端环境变量。浏览器客户端会自动：

- 生成稳定的 `X-Session-Id`
- 在报告读取、导出和流式请求里附带该 header
- 在 SSE 重连耗尽后退化为 `/api/v1/reports/{id}/status`
...
- 变更型请求仍然要求 `X-Requested-With`
- 报告读取、状态、流式和导出请求会带稳定的客户端 `X-Session-Id`
- 报告相关限流按 read、status、stream、mutation 分桶隔离
