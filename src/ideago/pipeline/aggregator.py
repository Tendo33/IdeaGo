...
def _fallback_uncertainty_note(output_language: str) -> str:
    if output_language == "zh":
        return "当前没有检索到足够证据，空结果不应被直接解读为存在明确市场空白。"
    return (
...
def build_failed_aggregation_result(
    *,
    ...
    return AggregationResult(
        ...
        recommendation_type=RecommendationType.CAUTION,
        uncertainty_notes=[
            _localized_failure_uncertainty_note(output_language),
        ],
    )
...
def _localized_failure_market_summary(output_language: str) -> str:
    if output_language == "zh":
        return "聚合分析失败，以下结论基于已提取的结构化证据进行保守降级生成。"
    return (
...
def _localized_failure_uncertainty_note(output_language: str) -> str:
    if output_language == "zh":
        return "聚合阶段失败，最终建议基于降级证据合成。"
    return "Aggregation stage failed; recommendation is based on degraded evidence synthesis."
