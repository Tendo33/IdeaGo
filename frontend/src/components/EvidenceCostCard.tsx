    ...
    <div className="card">
      ...
      <div className="space-y-2 mb-4">
        {topEvidence.length > 0 ? topEvidence.slice(0, 3).map((item, index) => (
          <div key={index} className="text-xs text-text-muted rounded-md bg-secondary/35 border border-border px-2.5 py-2">
            {item}
          </div>
        )) : (
      ...
      {expanded && (
        <div className="space-y-2 mb-4">
          {evidenceItems.slice(0, 6).map((item, index) => (
            <div key={`${item.title}-${index}`} className="rounded-md border border-border bg-secondary/20 px-3 py-2">
