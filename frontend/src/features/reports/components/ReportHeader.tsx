  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ref={triggerRef}
        onClick={() => setOpen(o => !o)}
        className="topbar-action text-sm font-medium focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none flex items-center justify-between gap-1 hover:bg-muted/50 rounded-none px-3 py-2 transition-colors duration-200"
        aria-haspopup="menu"
        aria-expanded={open}
        aria-controls={menuId}
      >
        {trigger}
        <ChevronDown className={`w-3.5 h-3.5 text-muted-foreground transition-transform duration-200 ease-out-quint ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div
          id={menuId}
...
function DropdownItem({ icon: Icon, label, onClick, href }: { icon: typeof Download; label: string; onClick?: () => void; href?: string }) {
  const cls = "flex items-center gap-2.5 w-full rounded-none px-3.5 py-2 text-sm text-muted-foreground hover:bg-muted/80 hover:text-foreground transition-colors duration-150 cursor-pointer focus-visible:bg-muted/80 focus-visible:text-foreground focus-visible:outline-none"
    ...
    <div className="flex flex-col gap-3 mb-6">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0 no-print w-full md:w-auto mt-2 md:mt-0">
          <Dropdown trigger={<div className="flex items-center gap-1.5"><Share2 className="w-4 h-4" /><span className="hidden sm:inline"> {t('report.header.share')}</span></div>}>
            <DropdownItem
          ...
          <Dropdown trigger={<div className="flex items-center gap-1.5"><Download className="w-4 h-4" /><span className="hidden sm:inline"> {t('report.header.export')}</span></div>}>
            <DropdownItem
              icon={Download}
